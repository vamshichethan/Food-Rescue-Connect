...
import { registerPushNotifications, sendLocalNotification } from '../lib/pushNotifications';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';


// Dynamically import Leaflet map (browser-only, no SSR)
const VolunteerMap = nextDynamic(() => import('../components/VolunteerMap'), { ssr: false });
...
export default function Dashboard() {
  ...
  const handleEnableNotifications = async () => {
    ...
    if (ok) {
      setNotifBanner(true);
      setTimeout(() => setNotifBanner(false), 3000);
      addLog('🔔 Browser push notifications enabled via Web Push API');
    }
  };
  ...
  const fireTestNotification = () => {
    sendLocalNotification(
      '🚨 Critical Food Alert!',
      'Surplus Baked Pastries (12.5 kg) — Spoilage risk: 82%. Claim now!',
      '/'
    );
    addLog('📣 Push notification dispatched to subscribed volunteer devices');
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <header className="glass" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          {notifBanner && (
            <div style={{ background: 'rgba(16,185,129,0.15)', border: '1px solid rgba(16,185,129,0.3)', color: 'var(--success)', padding: '6px 12px', borderRadius: '20px', fontSize: '0.72rem', fontWeight: 600 }}>
              ✅ Push notifications active!
            </div>
          )}
          <button
            onClick={notifEnabled ? fireTestNotification : handleEnableNotifications}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', borderRadius: '20px', fontSize: '0.72rem', border: `1px solid ${notifEnabled ? 'rgba(139,92,246,0.4)' : 'rgba(255,255,255,0.08)'}`, background: notifEnabled ? 'rgba(139,92,246,0.1)' : 'rgba(255,255,255,0.03)', color: notifEnabled ? 'var(--primary-hover)' : 'var(--text-muted)', cursor: 'pointer', fontWeight: 600 }}
          >
            {notifEnabled ? <BellRing size={12} /> : <Bell size={12} />}
            {notifEnabled ? 'Test Push Alert' : 'Enable Alerts'}
          </button>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
            ...
            {activeRole === 'volunteer' && (
              <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '30px' }}>
                ...
                <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  {/* Real Leaflet Map with OSM tiles + OSRM route */}
                  <Suspense fallback={<div style={{height: '240px', backgroundColor: '#07070a', borderRadius: '12px', border: '1px solid rgba(255, 255, 255, 0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)'}}>Loading live map...</div>}>
                    <VolunteerMap volunteerMoving={isVolunteerMoving} />
                  </Suspense>
            ...
            {activeRole === 'admin' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                ...
                {/* Advanced Recharts Carbon Impact Graph */}
                <div className="glass-card" style={{ padding: '24px', borderTop: '2px solid var(--success)' }}>
                  <h3 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Leaf size={18} color="var(--success)" /> Environmental Impact Trajectory
                  </h3>
                  <div style={{ width: '100%', height: '300px' }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart
                        data={[
                          { name: 'Mon', carbon: 45, meals: 20 },
                          { name: 'Tue', carbon: 52, meals: 25 },
                          { name: 'Wed', carbon: 38, meals: 18 },
                          { name: 'Thu', carbon: 65, meals: 30 },
                          { name: 'Fri', carbon: 85, meals: 42 },
                          { name: 'Sat', carbon: 110, meals: 50 },
                          { name: 'Sun', carbon: totalCO2Saved, meals: totalMealsServed },
                        ]}
                      ...
                      >
                        ...
                        <Tooltip 
                          contentStyle={{ backgroundColor: 'rgba(10,10,15,0.95)', border: '1px solid rgba(139,92,246,0.3)', borderRadius: '8px', fontSize: '0.75rem' }} 
                        />
                        <Area type="monotone" dataKey="carbon" name="CO₂ Offset (kg)" stroke="#10b981" fillOpacity={1} fill="url(#colorCarbon)" />
                        <Area type="monotone" dataKey="meals" name="Meals Served" stroke="#06b6d4" fillOpacity={1} fill="url(#colorMeals)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </div>
